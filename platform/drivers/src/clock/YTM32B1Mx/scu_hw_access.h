 ...
 *
 * @param[in] base  Base address for current SCU instance.
 * @param[in] autoGainDisable    enable/disable the external fast oscillator(FXOSC) ALC mode.
 *            true - OSC ALC is disabled
 *            false - OSC ALC is enabled
 */
...
/* Gain selection is only available on devices with GMSEL defined. */
