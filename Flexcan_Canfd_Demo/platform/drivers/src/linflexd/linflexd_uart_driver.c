...
* PRQA S 3387 Rule 13.3:  A full expression containing an increment (++) or decrement (--) 
*                         operator should have no potential side effects other than that 
...
static linflexd_uart_state_t * s_uartLINFlexDStatePtr[LINFlexD_INSTANCE_COUNT];
...
static status_t LINFlexD_UART_DRV_StartSendUsingDma(uint32_t instance, /* PRQA S 0779 */
                                                    const uint8_t * txBuff,
                                                    uint32_t txSize);
static status_t LINFlexD_UART_DRV_StartReceiveUsingDma(uint32_t instance, /* PRQA S 0779 */
                                                       uint8_t * rxBuff,
                                                       uint32_t rxSize);
static void LINFlexD_UART_DRV_CompleteSendUsingDma(void * parameter, dma_chn_status_t status); /* PRQA S 0779 */
static void LINFlexD_UART_DRV_CompleteReceiveUsingDma(void * parameter, dma_chn_status_t status); /* PRQA S 0779 */
#endif
                                ...
                                const linflexd_uart_user_config_t * uartUserConfig)
{
    ...
    status_t uartStatus = STATUS_SUCCESS;
    LINFlexD_Type * base;
    uint32_t idx;
    uint32_t uartSourceClock;
    clock_names_t instanceClkName;
...
status_t LINFlexD_UART_DRV_Deinit(uint32_t instance)
{
    ...
    /* Disable data tx and rx interrupts */
                                            ...
                                            uint32_t timeout)
{
    ...
    if (retVal == STATUS_SUCCESS)
    {
        ...
        if (retVal == STATUS_TIMEOUT)
        {
            ...
            {
                LINFlexD_UART_DRV_CompleteSendUsingDma(((void *)instance), DMA_CHN_NORMAL); /*PRQA S 0326*/
            }
#endif
        }
    }
                                            ...
                                            uint32_t txSize)
{
    ...
    if (uartState->isTxBusy)
    {
        ...
        while (uartState->txSize > 0U)
        {
            ...
            bufOverrunFlag = LINFlexD_GetStatusFlag(base, LINFlexD_UART_BUFFER_OVERRUN_FLAG);
            frameErrorFlag = LINFlexD_GetStatusFlag(base, LINFlexD_UART_FRAME_ERROR_FLAG);
            if((bufOverrunFlag) || (frameErrorFlag))
            {
        ...
        while(false == LINFlexD_GetStatusFlag(base, LINFlexD_UART_DATA_TRANSMITTED_FLAG))
        {
                                               ...
                                               uint32_t timeout)
{
    ...
    if (retVal == STATUS_SUCCESS)
    {
        ...
        if (retVal == STATUS_TIMEOUT)
        {
            ...
            {
                LINFlexD_UART_DRV_CompleteReceiveUsingDma(((void *)instance), DMA_CHN_NORMAL); /*PRQA S 0326*/
            }
#endif
        }
    }
                                               ...
                                               uint32_t rxSize)
{
    ...
    if (uartState->isRxBusy)
    {
        ...
        while ((uartState->rxSize > 0U) && (isRxError == false))
        {
...
status_t LINFlexD_UART_DRV_GoToSleepMode(uint32_t instance)
{
    ...
    LINFlexD_Type * base = s_uartLINFlexDBase[instance];
...
void LINFlexD_UART_DRV_RxIRQHandler(uint32_t instance)
{
    ...
    if (uartState->isRxBusy)
    {
        ...
        if (uartState->rxSize == 0U)
        {
            ...
            LINFlexD_UART_DRV_CompleteReceiveUsingInterrupts(instance);
            if(uartState->idleTimeoutState == LINFlexD_UART_TIMEOUT_PROCESSING)
            {
                uartState->idleTimeoutState = LINFlexD_UART_TIMEOUT_STOP;
            }
        ...
        {
            if(uartState->idleTimeoutState == LINFlexD_UART_TIMEOUT_PREPARE)
            {
                uartState->idleTimeoutState = LINFlexD_UART_TIMEOUT_START;
            }
        }
...
static void LINFlexD_UART_DRV_RxTOHandler(uint32_t instance)
{
    ...
    switch (uartState->idleTimeoutState)
    {
    case LINFlexD_UART_TIMEOUT_CLOSE:
        break;
    case LINFlexD_UART_TIMEOUT_PREPARE:
        break;
    case LINFlexD_UART_TIMEOUT_START:
        uartState->idleTimeoutState = LINFlexD_UART_TIMEOUT_PROCESSING;
        /* reset timeout counter by writting UARTPTO register */
        LINFlexD_SetIdleTimeoutValue(base, uartState->idleTimeoutCounterValue);
        ...
        /* code */
        break;
    case LINFlexD_UART_TIMEOUT_PROCESSING:
...
void LINFlexD_UART_DRV_IRQHandler(uint32_t instance)
{
    ...
    bool bufOverrunFlag = LINFlexD_GetStatusFlag(base, LINFlexD_UART_BUFFER_OVERRUN_FLAG);
    bool frameErrorFlag = LINFlexD_GetStatusFlag(base, LINFlexD_UART_FRAME_ERROR_FLAG);
    ...
    else 
    {
        ...
        if (LINFlexD_IsInterruptEnabled(base, (uint32_t)LINFlexD_DATA_RECEPTION_COMPLETE_INT))
        {
            if (LINFlexD_GetStatusFlag(base, LINFlexD_UART_DATA_RECEPTION_COMPLETE_FLAG))
            {
                                                              ...
                                                              uint32_t rxSize)
{
    ...
    if ((uartState->isRxBusy) && (uartState->rxCallback == NULL))
    {
        status = STATUS_BUSY;
    }else {
...
static void LINFlexD_UART_DRV_CompleteSendUsingDma(void * parameter, dma_chn_status_t status)
{
    ...
    instance = ((uint32_t)parameter); /*PRQA S 0326*/
    base = s_uartLINFlexDBase[instance];
    uartState = (linflexd_uart_state_t *)s_uartLINFlexDStatePtr[instance];
...
static void LINFlexD_UART_DRV_PutData(uint32_t instance)
{
    ...
    else
    {
        ...
        {
            LINFlexD_SetTxDataBuffer2Bytes(base, ((uint16_t)uartState->txBuff[1] << 8U) | (uint16_t)uartState->txBuff[0]);
...
static void LINFlexD_UART_DRV_GetData(uint32_t instance)
{
    ...
    else
    {
        ...
        {
            uartState->rxBuff[0] = (uint8_t)(LINFlexD_GetRxDataBuffer2Bytes(base) & 0xFFU);
            uartState->rxBuff[1] = (uint8_t)(LINFlexD_GetRxDataBuffer2Bytes(base) >> 8U);
            /* Move the pointer twice */
            ++uartState->rxBuff;
            ++uartState->rxBuff;
            uartState->rxSize -= 2U;
                                                ...
                                                linflexd_uart_idle_timeout_config_t * timeoutCounterConfig)
{
    ...
    if ((uartState->isTxBusy == true) || (uartState->isRxBusy == true))
    {
        status = STATUS_BUSY;
    }else {
        ...
        /* Configure idle timeout function */
        if(timeoutCounterConfig->enableIdleTimeout)
        {
            LINFlexD_SetTimeoutCounterResetMode(base, LINFlexD_UART_TIMEOUT_COUNTER_RESET_HW_MODE);
            LINFlexD_SetMonitorIdleState(base, true); 
            /* configure idle timeout counter value */
            LINFlexD_SetIdleTimeoutValue(base, timeoutCounterConfig->idleTimeoutCount);
            uartState->idleTimeoutState = LINFlexD_UART_TIMEOUT_PREPARE;
            uartState->idleTimeoutCounterValue = timeoutCounterConfig->idleTimeoutCount;
        }
        else
        {
            uartState->idleTimeoutState = LINFlexD_UART_TIMEOUT_CLOSE;   
        }
