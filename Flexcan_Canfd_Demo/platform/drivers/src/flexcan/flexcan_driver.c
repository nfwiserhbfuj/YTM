...
#if FEATURE_CAN_HAS_ENHANCE_RX_FIFO
static void FLEXCAN_CompleteRxEnhanceFifoDataDMA(void *parameter, 
                                          dma_chn_status_t status);
...
static uint8_t FLEXCAN_UseEnhanceRxFifoComputePayloadSize(uint8_t dlcValue);
#endif /* FEATURE_CAN_HAS_ENHANCE_RX_FIFO */
/*******************************************************************************
 * Code
 ******************************************************************************/
    ...
    uint32_t mask)
{
    ...
    {
        FLEXCAN_SetRxIndividualMask(base, mb_idx, mask);
    }
   ...
   const flexcan_user_config_t *data)
{
    ...
    if(result == STATUS_SUCCESS)
    {
        ...
        state->max_num_mb = data->max_num_mb;
                    ...
                    )
{
    ...
    CAN_Type * base = g_flexcanBase[instance];
    flexcan_state_t * state = g_flexcanStatePtr[instance];
    uint32_t dma_transfer_size = (uint32_t)(((base->ERFCR & CAN_ERFCR_DMALW_MASK) >> CAN_ERFCR_DMALW_SHIFT)+ 1U)* 4U;
    ...
    else
    {
        ...
        else if(state->transferType == FLEXCAN_RXFIFO_USING_DMA)
        {
            ...
            dmaStat = DMA_DRV_InstallCallback(state->rxFifoDMAChannel,
                                                FLEXCAN_CompleteRxEnhanceFifoDataDMA,
                                                (void *)((uint32_t)instance));/* PRQA S 0326 */
            ...
            {
                dmaStat = DMA_DRV_ConfigSingleBlockTransfer(state->rxFifoDMAChannel,
                                                                DMA_TRANSFER_MEM2MEM,
                                                                (uint32_t)((uint32_t)g_flexcanBase[instance] + FLEXCAN_ENHANCE_RX_FIFO_DATA_REGION_OFFSET),
                                                                (uint32_t)(state->enhanceRxFifoState.mb_message),
                                                                DMA_TRANSFER_SIZE_4B,
                                                                dma_transfer_size);
...
#if FEATURE_CAN_HAS_ENHANCE_RX_FIFO
static void FLEXCAN_CompleteRxEnhanceFifoDataDMA(void *parameter, dma_chn_status_t status)
{
    uint32_t instance = (uint32_t)parameter;/* PRQA S 0326 */
    ...
    FLEXCAN_CompleteRxMessageEnhanceFifoData((uint8_t)instance);
}
#endif /* FEATURE_CAN_HAS_DMA_ENABLE */
#endif
...
static void FLEXCAN_CompleteRxMessageEnhanceFifoData(uint8_t instance)
{
    ...
    CAN_Type * base = g_flexcanBase[instance];
    flexcan_state_t * state = g_flexcanStatePtr[instance];
    uint8_t EnhanceRxFifoDataByte = 0U;
    uint32_t EnhanceRxFifoDataWord = 0U;
    ...
    {
        if (state->enhanceRxFifoState.state != FLEXCAN_MB_DMA_ERROR)
        {
            flexcan_msgbuff_t *fifo_message = state->enhanceRxFifoState.mb_message;
            uint32_t *msgData_32 = (uint32_t *)fifo_message->data;/* PRQA S 0310 */ /* PRQA S 3305 */
            ...
            EnhanceRxFifoDataByte = FLEXCAN_UseEnhanceRxFifoComputePayloadSize(fifo_message->dataLen);
            /* Reverse the endianness */
            /* Copy MB[0] data field into user's buffer */
            for (uint8_t i = 0U; i < EnhanceRxFifoDataByte; i += 4U)
            {
                EnhanceRxFifoDataWord = msgData_32[i >> 2U];  /* PRQA S 2983 */
                FlexcanSwapBytesInWord(EnhanceRxFifoDataWord, msgData_32[i >> 2U]);
            }
        }
    }
#endif
    ...
    /* Update status for receive by using enhance RX FIFO*/
