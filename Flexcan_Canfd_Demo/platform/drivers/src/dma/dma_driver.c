...
/*! @brief Dummy channel state structure to clear extra request. */
static dma_chn_state_t s_dmaDummyConfigState;
...
#if defined(FEATURE_DMA_ERRATA_E406002) && (FEATURE_DMA_ERRATA_E406002 == 1)
    if((s_virtEdmaState->maxChannelForChLinkState)
        && (virtualChannel == s_virtEdmaState->virtChnState[FEATURE_DMA_VIRTUAL_CHANNELS-1U]->virtChn))
    {
        /* If link channel is intended to clear extra requests., it not have chnState, so it is not required clear structure. */
    }
    else
    {
